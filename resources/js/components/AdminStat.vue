         ...
         <v-flex xs8 sm6 md4 lg3 lx2>
          ...
          >           
            <template slot="items" slot-scope="props">
              ...
              <td>{{ props.item.data }}
                <!-- ul class="aspire_ul">
                  <li  v-for="(value, key) in  JSON.parse(props.item.data)" :value="value" :key="key">
                    {{key}}: <span class="blue--text">{{value}}</span> 
                  </li>
                </ul -->
              </td>
              <td>{{ props.item.added_on }}</td>
            </template>
...
<script>
  export default {   
    ...
    mounted: function() {
      ...
      this.obj_stat = JSON.parse(this.stat);
      for(let i=0; i<this.obj_stat.length; i++){
       // console.log(this.obj_stat[i].website_addr);
       // console.log(JSON.parse(this.obj_stat[i].data).HTTP_ORIGIN);
        this.obj_stat[i].website_addr = JSON.parse(this.obj_stat[i].data).HTTP_ORIGIN;
      }
      this.datas.sheet = JSON.parse(this.stat);     
    },   
    methods: {
