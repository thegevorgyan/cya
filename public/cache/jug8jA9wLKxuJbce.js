...
function aspire_loadHtml(elm, key = "default"){	
	...
	xhttp.open('GET', 'http://localhost/reg/public/cache/jug8jA9wLKxuJbce.html', true);	
...
function aspire_processData(val){
	...
	xhttp.open('GET', 'http://localhost/reg/public/check?' + collectedDataString, true);
	xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
	xhttp.onreadystatechange = function() {
				...
				if(json.status == "200"){
					...
					setTimeout(function(){
						...
						setTimeout(function(){
							...
							xhr.open('GET', 'http://localhost/reg/public/storage?' + storedDataString, true);
...
function showPopup(){
	var fileref=document.createElement("link");
			...
			fileref.setAttribute("href", 'http://localhost/reg/public/cache/jug8jA9wLKxuJbce.css');
...
function ready(){
	...
	if(!created_new_token){
		...
		var xhttp = new XMLHttpRequest();
		xhttp.open('GET', 'http://localhost/reg/public/compare?' + collectedDataString, true);
		xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhttp.onreadystatechange = function() {		
			if (this.readyState == 4 && this.status == 200) {
