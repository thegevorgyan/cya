...
function aspire_loadHtml(elm, key = "default"){	
	...
	xhttp.open('GET', 'http://localhost/cya/public/cache/XkLU2i7rANDaR6WQ.html', true);	
...
/*function myFunction() {
	var windowHeight = window.innerHeight;		
	 var modalHeight = document.getElementsByClassName('aspire_modal_content').offsetHeight;
// var txt = x += 1;
//		 document.getElementById("text").innerHTML = txt;
//			document.getElementById('text').innerHTML='window='+windowHeight+'modal='+modalHeight;

	if(windowHeight<modalHeight){
		document.getElementsByClassName("aspire_modal_content")[0].style.overflowY='auto';
	} else {
		document.getElementsByClassName("aspire_modal_content")[0].style.overflowY='hidden';
	}
...
function aspire_processData(val){
	...
	xhttp.open('GET', 'http://localhost/cya/public/check?' + collectedDataString, true);
	xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
	xhttp.onreadystatechange = function() {
				...
				if(json.status == "200"){
					...
					setTimeout(function(){
						...
						setTimeout(function(){
							...
							xhr.open('GET', 'http://localhost/cya/public/storage?' + storedDataString, true);
...
function showPopup(){
	var fileref=document.createElement("link");
			...
			fileref.setAttribute("href", 'http://localhost/cya/public/cache/XkLU2i7rANDaR6WQ.css');
...
function ready(){
	...
	if(!created_new_token){
		...
		var xhttp = new XMLHttpRequest();
		xhttp.open('GET', 'http://localhost/cya/public/compare?' + collectedDataString, true);
		xhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
		xhttp.onreadystatechange = function() {		
			if (this.readyState == 4 && this.status == 200) {
